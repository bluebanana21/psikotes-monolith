...
function parseArgs(): Partial<Bun.BuildConfig> {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    // Handle boolean flags and flags without values
    const nextArg = args[i + 1];
    if (!arg.includes("=") && (i === args.length - 1 || nextArg?.startsWith("--"))) {
    ...
    if (arg.includes("=")) {
      ...
      value = args[i] ?? ""; // Use nullish coalescing to handle undefined
    }
